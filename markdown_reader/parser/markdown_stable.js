// Combined Markdown Parser with Indentation Support and Syntax Highlighting
(function() {
    // ==================================================================
    // CODEBLOCKS MODULE
    // ==================================================================
    ...
    // Restore with syntax highlighting
    function restoreCodeBlocks(t, s, hf) {
        ...
        r = r.replace(rFP, (m, i) => {
            ...
            if (window.MarkdownSyntaxHighlighter) {
                if (lang === 'markdown' || lang === 'md' || lang === 'mkd') {
                    // Markdown code blocks get full syntax highlighting
                    cnt = MarkdownSyntaxHighlighter.highlight(cnt);
                } else if (lang) {
                    // Language specified but not markdown - use highlight.js if available
                    if (window.hljs) {
                        try {
                            cnt = hljs.highlight(cnt, { language: lang }).value;
                        } catch (e) {
                            // Fallback to neutral if highlight fails
                            cnt = escapeHtml(cnt);
                        }
                    } else {
                        // No highlighter - just escape
                        cnt = escapeHtml(cnt);
                    }
                } else {
                    // No language specified - neutral colors only
                    cnt = escapeHtml(cnt);
                }
            } else if (hf) {
                cnt = hf(cnt, lang);
            ...
            return '<pre><code' + (lang ? ` class="language-${lang}"` : '') + '>' + cnt + '</code></pre>';
        });
        ...
        r = r.replace(rIP, (m, i) => {
            ...
            if (window.MarkdownSyntaxHighlighter && sp.lang === 'markdown') {
                cnt = MarkdownSyntaxHighlighter.highlight(cnt);
            } else if (hf) {
                cnt = hf(cnt, 'inline');
            } else {
    ...
    // Main render function - Fixed for images, bold/italic in links, and link titles
    function ri(md, rm, fd, fn, fro, frc) {
        ...
        // STEP 1: Process images FIRST - completely separate from links
        // Use a different placeholder for images
        ...
        // STEP 2: Process emphasis (bold, italic, etc.) - affects link text
        ...
        // STEP 3: Process links with placeholders
        const linkPlaceholders = [];
        let linkCount = 0;
        
        res = res.replace(rL, (m) => {
            const placeholder = `!!LINK_${linkCount}!!`;
        ...
        // Process autolinks
        res = res.replace(rAU, (m, au, pu, em) => {
            if (au) {
        ...
        // STEP 4: Restore links with proper title handling
        res = res.replace(/!!LINK_(\d+)!!/g, (m, index) => {
            const link = linkPlaceholders[parseInt(index)];
            
            // Parse the link using the original regex
            const linkMatch = link.match(/^\[([^\]]+)\]\(([^)\s]+)(?:\s+(["'][^"']*["']|\([^)]*\)))?\)$/);
            if (!linkMatch) return link; // Shouldn't happen
            
            const linkText = linkMatch[1]; // Text already has emphasis applied
            const url = linkMatch[2];
            const titlePart = linkMatch[3];
            
            const su = e(url);
            let ta = '';
            
            if (titlePart) {
                // Clean up the title - remove outer quotes/parentheses
            ...
            return `<a href="${su}"${ta}>${linkText}</a>`;
        });

        // STEP 5: Restore images
        res = res.replace(/!!IMG_(\d+)!!/g, (m, index) => {
            const img = imgPlaceholders[parseInt(index)];
            const su = e(img.url);
            const sa = e(img.alt);
            let ac = '';
            
            if (img.lc && img.rc) ac = ' c';
            else if (img.rc) ac = ' r';
            else if (img.lc) ac = ' l';
            
            const em = img.url.match(/\.([^.]+)$/);
            if (em) {
    ...
    function parseMarkdown(text) {
        ...
        const nl = lines.map((line) => {
            ...
            return {
                text: line.trim(),
                rawIndent: im ? im[1] : '', // Store raw indentation string
                ri: ri,
                nl: i2l[ri] || 0,
                ie: false
            };
        ...
        for (let i = 0; i < nlen; i++) {
            ...
            const line = it.text;
            const ri = it.ri;
            const rawIndent = it.rawIndent || ''; // Get raw indentation
            ...
            if (lst.length > 0) {
                const tri = lst[lst.length - 1].ri;
                if (it.ri > tri) {
                    // This paragraph is indented under the current list item
                    // Use the indentation from the first line
                    let pl = [line];
                    ...
                    // Paragraph at same level as list - close the list
                    while (lst.length > 0) {
                        const cl = lst.pop();
                        lr_.push('</li>');
                        lr_.push('</' + cl.t + '>');
                ...
                // Regular paragraph outside lists
                // Check if there are multiple lines to group
                let pl = [line];
                let firstIndent = rawIndent;
                let j = i + 1;
        ...
        // Final pass: Apply syntax highlighting to any remaining code blocks
        if (window.MarkdownSyntaxHighlighter || window.hljs) {
            ...
            codeBlocks.forEach(block => {
                ...
                if (lang === 'markdown' || lang === 'md' || lang === 'mkd') {
                    ...
                    if (window.MarkdownSyntaxHighlighter) {
                        ...
                        const highlighted = MarkdownSyntaxHighlighter.highlight(code);
                        block.innerHTML = highlighted;
                    }
                } else if (lang && window.hljs) {
                    // Other languages use highlight.js
                    try {
                ...
                // No language = neutral (already escaped)
            });
